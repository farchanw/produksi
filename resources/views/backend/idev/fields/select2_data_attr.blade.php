...
<div class="{{(isset($field['class']))?$field['class']:'form-group'}}">
    ...
    <select
        ...
        name="{{$select_name}}"
        class="form-control idev-form support-live-select2 support-live-select2-data-attr @if($prefix_repeatable) field-repeatable @endif">
        ...
        @foreach($field['options'] as $key => $opt)
            ...
            <option value="{{$opt['value']}}"
                ...
                @foreach($opt as $attr => $val)
                    @if(str_starts_with($attr, 'data_') || str_starts_with($attr, 'data-'))
                        {{ str_replace('_', '-', $attr) }}="{{ $val }}"
                    @endif
                @endforeach
