...
<div class="{{(isset($field['class']))?$field['class']:'form-group'}} field-number-text-group-{{(isset($field['name']))?$field['name']:'name_'.$key}}">
    ...
    <div class="d-flex gap-2">
        <select id="{{$preffix_method}}{{(isset($field['name']))?str_replace("[]","",$field['name']):'id_'.$key}}_select" 
            name="{{(isset($field['name']))?$field['name']:'name_'.$key}}_type" 
            class="form-control idev-form form-select w-auto"
        >
            @foreach($field['options'] as $key => $opt)
                <option value="{{$opt['value']}}" @if(isset($opt['checked'])) selected @endif>{{$opt['text']}}</option>
            @endforeach
        </select>
